...
def DeleteAny(config, signer, Compartments, ServiceClient, ServiceName, ServiceID="", ReturnServiceID="id",
              ListCommand="", GetCommand="", DeleteCommand="", ObjectNameVar="display_name", DelState="DELETED",
              DelingSate="DELETING", Extra="", Filter="", PerAD=False):
    AllItems = []
    ...
    for C in Compartments:
        ...
        try:
            if PerAD:
                ...
                for ad in ads:
                    itemstemp = eval(
                        "oci.pagination.list_call_get_all_results(object.{}, availability_domain=\"{}\", compartment_id=Compartment.id{}, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY).data".format(
                            ListCommand, ad.name, Extra))
                    for item in itemstemp:
                        items.append(item)
            else:
                items = eval(
                    "oci.pagination.list_call_get_all_results(object.{}, compartment_id=Compartment.id{}, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY).data".format(
                        ListCommand, Extra))
        ...
        for item in items:
            ...
            if DelState == "":
                try:
                    print(
                        "Deleting {}: {}-{} @ {}".format(C.fullpath, ServiceName, eval("item.{}".format(ObjectNameVar)),
                                                         config["region"]))
                    eval("object.{}({}=item.{}, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)".format(DeleteCommand,
                                                                                                         ServiceID,
                                                                                                         ReturnServiceID))
                except oci.exceptions.ServiceError as response:
                    if response.code == 404:
                        ...
                        print(
                            "error {}-{} trying to delete: {} - {}".format(response.code, response.message, C.fullpath,
                                                                           eval("item.{}".format(ObjectNameVar))))
                        ...
                        try:
                            ...
                            if itemstatus.lifecycle_state.lower() != DelState.lower():
                                if itemstatus.lifecycle_state.lower() != DelingSate.lower():
                                    try:
                                        print("Deleting {}: {}-{} @ {}".format(C.fullpath, ServiceName, eval(
                                            "itemstatus.{}".format(ObjectNameVar)), config["region"]))
                                        eval(
                                            "object.{}({}=itemstatus.{}{}, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)".format(
                                                DeleteCommand, ServiceID, ReturnServiceID, Extra))
                                    except oci.exceptions.ServiceError as response:
